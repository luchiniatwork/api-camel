...
export default class BaseEndpoints {
  ...
  constructor(options) {
    ...
    this._q = require('q');
    this._debug = this._debug('api-camel:base-endpoint');

    options = options || {};
    this._modelList = options.modelList;
    this._baseURL = options.baseURL;
    
    this._debug('initialized with', options);
  }
  
  _getModelConstructor(modelMapId) {
    this._debug(modelMapId, 'looking for meta objects');
    var out = {},
        regExpStr = null,
        construct = null,
        matchingType = null,
      ...
      for (var mapIdx in mapIds) {
        ...
        if (mapRegexp.test(modelMapId)) {
          if (matchingType === 'DIRECT') {
            directCount++;
            this._debug(modelMapId, 'found DIRECT model match:', regExpStr);
          } else {
            hashedCount++;
            this._debug(modelMapId, 'found HASHED model match:', regExpStr, 'hashing to:', hash);
          }
  ...
  _sanitizeUnmetHigherLevels(model) {
    var out = null;
  ...
  _buildModel(src, modelMapId, options) {
      ...
      if (_.isArray(src)) {
        ...
        if (_.keys(metaObjs).length > 0) {
          ...
          if (metaObj) {
            ...
            this._debug(modelMapId, '... instantiating found model');
          } else {
            model = new BaseModel();
          }
        } else {
          ...
          if (newVal) {
            if (
              ...
              for (var idx in hashedMetaObjs) {
                ...
                this._debug('setting hash', hashedKey, 'with value', hashedVal);
            ...
            if (_.isArray(model)) {
              model.push(newVal);
              this._debug(modelMapId, 'pushing to array for', modelMapId);
            } else {
  ...
  _request(options) {
    ...
    this._debug('firing request', url);
