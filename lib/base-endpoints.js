...
BaseEndpoints.prototype._buildModel = function (src, modelMapId) {
  ...
  if (src) {
    if (_.isArray(src)) {
      debug('creating array', modelMapId);
      model = [];
      ...
      if (modelConstructor) {
        ...
        debug('found model object for', modelMapId);
      } else {
        // debug('no model found for', modelMapId);
        model = new BaseModel();
      }
    }
    
    for (var key in src) {
      if (key === 'g') debug('key', key);
      // debug('key', key, 'src', src);
      var val = src[key];
      if (
        _.isObject(val) ||
        _.isArray(val)
        ...
        if (newVal) {
          if (_.isArray(model)) {
            if (modelMapId === 'TeamScheduleResponse::gscd::g') debug('pushing', newVal, 'for', key);
            model.push(newVal);
          } else {
            if (modelMapId === 'TeamScheduleResponse::gscd::g') debug('setting', key);
